...
/* Операторы try могут быть неявно вложены
   в вызовы методов.
 */
class MethNestTry {
    static void nestTry(int a) {
        try { // Вложенный блок оператор try
            /* Если указан один аргумент командной строки,
               то иключение в связи с делением на нуль
               будет сгенерировано в следующем коде.
             */
            if (a == 1)
                a = a / (a - a); // деление на нуль
            // Исключение в данном случае будет сгенерировано за пределами
            // вложенного оператор try.
    ...
    public static void main(String[] args) {
        try {
            int a = args.length; // Для проверки используй a = 1, a = 2,
                                 // чтобы проверить вложенный оператор try
